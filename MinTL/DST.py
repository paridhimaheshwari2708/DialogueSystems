...
class MultiWozReader(_ReaderBase):
	...
	def wrap_result_with_response(self, result_dict):
		decode_fn = self.vocab.sentence_decode
		...
		for dial_id, turns in result_dict.items():
			results[dial_id] = []
			for turn_no, turn in enumerate(turns):
				entry = {'dial_id': dial_id, 'turn_num': turn_no}
				for key in field:
					if key in ['dial_id', 'turn_num']:
						continue
					v = turn.get(key, '')
					decoded = decode_fn(v)
					if key == 'user':
						decoded = decoded.split('<eos_r>')[-1]
					entry[key] = decoded
				results[dial_id].append(entry)
		return results
...
class Model(object):
	...
	def eval(self, data='test'):
		...
		result_collection = {}
		data_iterator = self.reader.get_batches(data)
		for batch_num, dial_batch in enumerate(tqdm(data_iterator)):
			py_prev = {'bspn': None}
			for turn_num, turn_batch in enumerate(dial_batch):
				first_turn = (turn_num==0)
				inputs = self.reader.convert_batch(turn_batch, py_prev, first_turn=first_turn, dst_start_token=self.model.config.decoder_start_token_id)
		...
		results_with_response = self.reader.wrap_result_with_response(result_collection)
		with open(os.path.join(self.args.model_path, 'predictions.json'), 'w') as f:
			json.dump(results_with_response, f)
