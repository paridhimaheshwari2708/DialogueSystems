...
def load_and_cache_examples(args, model, tokenizer, processor, evaluate=False):
    ...
    # logger.info("Cached file is at %s", cached_file)
    # logger.info("os.path.dirname(args.output_dir) at %s", os.path.dirname(args.output_dir))
    # logger.info("args.output_dir at %s", args.output_dir)
    # logger.info("Cached file is at %s", cached_file)

    cached_file = args.cached_file
    cached_train_examples = args.cached_train_examples
    logger.info("Cached file is at %s", cached_file)
    if os.path.exists(cached_file) and not args.overwrite_cache: # and not output_examples:
        ...
        if evaluate and args.predict_type == "dev":
            ...
            if os.path.exists(cached_train_examples) and not args.overwrite_cache: # and not output_examples:
                logger.info("Loading examples from cache -  %s", cached_train_examples)
                with open(cached_train_examples, 'rb') as f_ex:
                    examples = pickle.load(f_ex)
...
def main():
    ...
    parser.add_argument("--cached_file", default=None, type=str, required=True,
                        help="This is used to cache the features calculated for the data")
    parser.add_argument("--cached_train_examples", default="", type=str,
                        help="This is used to cache the features calculated for the data")
