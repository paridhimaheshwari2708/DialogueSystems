...
class rotator:
    def __init__(self, asent, aloi = [u"nsubj", u"dobj", u"iobj", u"obj", u"obl"],
        ...
        self.sent = asent
        self.cnkr = chunker.chunker(asent, aloi, pl, multilabs)
        ...
        if len(self.flex_chunks) > 1:
            ...
            for chunk_order in poss_perms:
                ...
                if num <= self.threshold:
                    ...
                    new_sent = self.sent.print_sent_ord(neworder)
                    newrows = self.sent.reorder(neworder)
                    shufledSentsRows.append(newrows)
                    shufledSentstext.append(new_sent)
        return shufledSentsRows, shufledSentstext
        ...
        if len(self.flex_chunks) > 1:
            ...
            for i in range(len(self.flex_chunks)):
                ...
                if num <= self.threshold:
                    ...
                    newrows = self.sent.reorder(neworder)
                    new_sent = self.sent.print_sent_ord(neworder)
                    croppedSentRows.append(newrows)
                    CroppedSentTexts.append(new_sent)
                    
        return croppedSentRows, CroppedSentTexts